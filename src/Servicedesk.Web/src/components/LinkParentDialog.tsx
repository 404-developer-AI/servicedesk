...
type Props = {
  ...
  source: Ticket;
  onClose: () => void;
  onLinked?: (parentTicketId: string) => void;
};

/// Picker dialog for manually linking the current ticket as a sub-ticket
/// of another. Non-destructive — both tickets stay independently editable;
/// the link is just a `parent_ticket_id` pointer. Server-side validates
/// for self-link, cycles, merged-state on either side and queue access.
export function LinkParentDialog({
  ...
  // Same debounced /api/tickets/picker call as MergeTicketDialog. Picker
  // already excludes the source itself.
  React.useEffect(() => {
    if (!open) return;
  ...
  const linkMutation = useMutation({
    mutationFn: () => ticketApi.linkParent(source.id, selected!.id),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ["ticket", source.id] });
      queryClient.invalidateQueries({ queryKey: ["ticket", response.parentTicketId] });
      toast.success(`Linked to main ticket #${response.parentNumber}`);
      onLinked?.(response.parentTicketId);
      onClose();
    },
    onError: (err) => {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && !linkMutation.isPending && onClose()}>
      <DialogContent className="flex max-h-[90vh] w-[calc(100vw-2rem)] max-w-3xl flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Mark this ticket as a sub-ticket of another. Both tickets stay
            editable; only the relationship is recorded.
          </DialogDescription>
        </DialogHeader>
