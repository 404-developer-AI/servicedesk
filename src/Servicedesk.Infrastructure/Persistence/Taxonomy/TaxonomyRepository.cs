...
public sealed class TaxonomyRepository : ITaxonomyRepository
{
    ...
    {
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        var row = await conn.QueryFirstOrDefaultAsync<(bool IsSystem, int TicketCount)>(new CommandDefinition("""
            SELECT q.is_system AS IsSystem,
        ...
        if (row == default) return DeleteResult.NotFound;
    ...
    {
        const string sql = """
            INSERT INTO priorities (name, slug, level, color, icon, sort_order, is_active, is_system)
            VALUES (@Name, @Slug, @Level, @Color, @Icon, @SortOrder, @IsActive, @IsSystem)
            ...
            """;
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        return await conn.QuerySingleAsync<Priority>(new CommandDefinition(sql, p, cancellationToken: ct));
    }

    public async Task<Priority?> UpdatePriorityAsync(Guid id, string name, string slug, int level, string color, string icon, int sortOrder, bool isActive, CancellationToken ct)
    {
        const string sql = """
            ...
            """;
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        return await conn.QueryFirstOrDefaultAsync<Priority>(new CommandDefinition(sql,
            new { id, name, slug, level, color, icon, sortOrder, isActive }, cancellationToken: ct));
    }
    ...
    public async Task<DeleteResult> DeletePriorityAsync(Guid id, CancellationToken ct)
    {
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        var row = await conn.QueryFirstOrDefaultAsync<(bool IsSystem, int TicketCount)>(new CommandDefinition("""
            SELECT p.is_system AS IsSystem,
    ...
    public async Task<DeleteResult> DeleteStatusAsync(Guid id, CancellationToken ct)
    {
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        var row = await conn.QueryFirstOrDefaultAsync<(bool IsSystem, int TicketCount)>(new CommandDefinition("""
            SELECT s.is_system AS IsSystem,
    ...
    {
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        var row = await conn.QueryFirstOrDefaultAsync<(bool IsSystem, int TicketCount, int ChildCount)>(new CommandDefinition("""
            SELECT c.is_system AS IsSystem,
