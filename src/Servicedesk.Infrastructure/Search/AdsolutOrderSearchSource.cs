...
public sealed class AdsolutOrderSearchSource : ISearchSource
{
        ...
        SearchRequest request, SearchPrincipal principal, CancellationToken ct)
    {
        ...
        // Feature-flag gate: a user without adsolut_orders_enabled gets zero
        // hits, exactly like the overview is hidden from them.
        var enabled = await conn.ExecuteScalarAsync<bool>(new CommandDefinition(
            "SELECT COALESCE(adsolut_orders_enabled, FALSE) FROM users WHERE id = @userId",
