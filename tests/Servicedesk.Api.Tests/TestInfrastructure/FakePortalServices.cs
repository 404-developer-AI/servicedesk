...
public sealed class FakePortalAccountRepository : IPortalAccountRepository
{
    ...
    public Task<Guid> CreateTokenAsync(string kind, byte[] tokenHash, string email, Guid? userId, Guid? contactId, Guid? companyId, string? companyRole, string displayName, Guid? createdByUserId, DateTime expiresUtc, CancellationToken ct) => Task.FromResult(Guid.NewGuid());
...
{
    public Task<PortalTicketPage> ListAsync(PortalViewer viewer, PortalTicketFilter filter, string? search, int page, int pageSize, CancellationToken ct) =>
...
public sealed class FakePortalAccountService : IPortalAccountService
{
    ...
    public Task<ApproveResult> ApproveAsync(Guid userId, Guid? companyId, string companyRole, PortalActor actor, CancellationToken ct) => throw NotWired();
    public Task<bool> RejectAsync(Guid userId, string? reason, PortalActor actor, CancellationToken ct) => throw NotWired();
    public Task<InviteResult> InviteAsync(string email, string displayName, Guid? contactId, Guid? companyId, string companyRole, PortalActor actor, CancellationToken ct) => throw NotWired();
