...
public sealed class TicketTimeAlertServiceTests
{
    ...
    public async Task Extend_without_customer_confirmation_is_rejected()
    {
        ...
        var result = await service.ExtendAsync(
            Guid.NewGuid(), Guid.NewGuid(), addMinutes: 60, customerConfirmed: false);
    ...
    public async Task Extend_with_invalid_minutes_is_rejected(int minutes)
    {
        ...
        var result = await service.ExtendAsync(
            Guid.NewGuid(), Guid.NewGuid(), addMinutes: minutes, customerConfirmed: true);
    ...
    {
        // Lazy data source — never connected because both guarded paths
        // short-circuit before opening a connection. Construction alone does
        // not connect.
        var ds = NpgsqlDataSource.Create("Host=localhost;Username=u;Password=p;Database=d");
        return new TicketTimeAlertService(ds, new StubSettings());
    }
