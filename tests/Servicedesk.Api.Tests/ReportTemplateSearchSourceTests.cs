...
/// Pins the authorization boundary of <see cref="ReportTemplateSearchSource"/>:
/// customers see nothing, agents/admins are eligible. The per-user feature-flag
/// filter (<c>contracts_enabled</c>) that yields zero hits for an agent without
/// the flag is enforced inside SearchAsync via a DB read, exercised against a
/// real Postgres in the integration pass. These unit-level tests cover the
/// availability gate + the no-DB short-circuits (passing null! deps proves a
/// Customer or empty query never opens a connection).
public sealed class ReportTemplateSearchSourceTests
{
    [Fact]
    public void Customer_principal_is_not_available()
    {
        var src = new ReportTemplateSearchSource(null!);
    ...
    public async Task Customer_search_returns_empty_group_without_hitting_db()
    {
        ...
        var result = await src.SearchAsync(
            new SearchRequest("report", null, 10, 0), principal, default);

        Assert.Equal(SearchSourceKind.ReportTemplates, result.Kind);
    ...
    {
        Assert.Equal("report-templates", SearchSourceKind.ReportTemplates);
    }
}
